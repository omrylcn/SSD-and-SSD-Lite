...
from keras.layers import Input, Lambda, Conv2D, MaxPooling2D, BatchNormalization, ELU, Reshape, Concatenate, Activation,ReLU,add,Dropout,GlobalAveragePooling2D,DepthwiseConv2D
...
def conv_block(inputs, filters, kernel, strides):
    ...
    x = Conv2D(filters, kernel, padding='same')(inputs)
    x = BatchNormalization(axis=channel_axis)(x)
    return ReLU(6)(x)


def bottleneck(inputs, filters, kernel, t, s, r=True):
    channel_axis = 1 if K.image_data_format() == 'channels_first' else -1
    ...
    if r and s==1:
        res=inputs
        if inputs.shape[3] != filters and s==1:
            res = Conv2D(filters, kernel_size=1, strides=s, padding="same",kernel_initializer="he_normal")(res)
        x = add([x, res])
    return x
...
def build_model(image_size,
    ...
    ############################################################################
    alpha=1
    i = Input(shape=(img_height, img_width, img_channels))

    # The following identity layer is only needed so that the subsequent lambda layers can be optional.
    x = Lambda(identity_layer, output_shape=(img_height, img_width, img_channels), name='identity_layer')(i)
    ...
    if swap_channels:
        x = Lambda(input_channel_swap, output_shape=(img_height, img_width, img_channels), name='input_channel_swap')(x)
   
        
    x = Conv2D(filters, kernel, padding='same')(x)
    x = BatchNormalization(axis=channel_axis)(x)
    x = inverted_residual_block(x, int(32*alpha), (3, 3), t=2, strides=1, n=1)
    x = inverted_residual_block(x, int(48*alpha), (3, 3), t=3, strides=1, n=1)
    x = inverted_residual_block(x, int(64*alpha), (3, 3), t=4, strides=2, n=1)
    x = inverted_residual_block(x, int(64*alpha), (3, 3), t=4, strides=1, n=1)
    x = inverted_residual_block(x, int(128*alpha), (3, 3), t=4, strides=2, n=1)
    conv4 = inverted_residual_block(x, int(128*alpha), (3, 3), t=6, strides=1, n=1)
    x = inverted_residual_block(conv4, int(128*alpha), (3, 3), t=6, strides=2, n=1)
    conv5 = inverted_residual_block(x, int(256*alpha), (3, 3), t=6, strides=1, n=1)
    x,ek=LiteConv(conv5,1,512)
    x,conv6=LiteConv(x,2,256)
    x,ek=LiteConv(x,3,128)
    x,conv7=LiteConv(x,4,64)
    ...
    if mode == 'training':
        ...
        model = Model(inputs=i, outputs=decoded_predictions)
