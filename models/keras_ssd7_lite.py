...
def _bottleneck(inputs, filters, kernel, t, s, r=True):
  
    channel_axis = 1 if K.image_data_format() == 'channels_first' else -1
    print(channel_axis,inputs)
...
def build_model(image_size,
    ...
    x1 = Lambda(identity_layer, output_shape=(img_height, img_width, img_channels), name='identity_layer')(i)
    ...
    alpha=1
    x = _conv_block(x1, int(32*alpha), (3, 3), strides=(1, 1))
    x = _inverted_residual_block(x, int(32*alpha), (3, 3), t=2, strides=1, n=1)
    x = _inverted_residual_block(x, int(48*alpha), (3, 3), t=3, strides=1, n=1)
    x = _inverted_residual_block(x, int(64*alpha), (3, 3), t=4, strides=2, n=1)
    x = _inverted_residual_block(x, int(64*alpha), (3, 3), t=4, strides=1, n=1)
    x = _inverted_residual_block(x, int(128*alpha), (3, 3), t=4, strides=2, n=1)
    conv4 = _inverted_residual_block(x, int(128*alpha), (3, 3), t=6, strides=1, n=1)
    x = _inverted_residual_block(conv4, int(128*alpha), (3, 3), t=6, strides=2, n=1)
    conv5 = _inverted_residual_block(x, int(256*alpha), (3, 3), t=6, strides=1, n=1)
